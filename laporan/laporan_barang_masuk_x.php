...
$pdf->SetX(1.6);
$pdf->SetFont('Times','i',10);
$pdf->MultiCell(22.5,0.6,"Laporan Barang Masuk: ".$ta."- ".$ak,0,'L'); 
...
$pdf->Cell(3, 0.6, 'Tgl. Barang Masuk', 1, 0, 'C');
$pdf->Cell(3, 0.6, 'Jml. Barang', 1, 1, 'C');

$no=1;
$sql="SELECT * FROM tbl_barang_masuk WHERE tgl_barang_masuk between '$ta' AND '$ak'";
...
while($lihat=mysqli_fetch_array($tampil)){
    ...
    $pdf->Cell(3, 0.6, $lihat['stok_awal'],1, 1, 'L');
    $no++;
}
$pdf->SetFont('Times','B',7);
$order="SELECT * FROM tbl_barang_masuk WHERE tgl_barang_masuk between '$ta' AND '$ak'";
...
$pdf->Cell(25, 0.8,"Jumlah Barang Masuk",1, 0, '');
