...
$pdf->SetX(1.6);
$pdf->SetFont('Times','i',10);
$pdf->MultiCell(22.5,0.6,"Laporan Return Barang: ".$ta."- ".$ak,0,'L'); 
...
$pdf->Cell(3, 0.6, 'Tgl. Return Barang', 1, 0, 'C');
$pdf->Cell(2, 0.6, 'Jml. Barang', 1, 0, 'C');
$pdf->Cell(4, 0.6, 'Keterangan', 1, 1, 'C');

$no=1;
$sql="SELECT * FROM tbl_return_barang WHERE tgl_return_barang between '$ta' AND '$ak'";
...
while($lihat=mysqli_fetch_array($tampil)){
    ...
    $pdf->Cell(3, 0.6, $lihat['tgl_return_barang'],1, 0, 'L');
    $pdf->Cell(2, 0.6, $lihat['jml_return_barang'],1, 0, 'L');
    $pdf->Cell(4, 0.6, $lihat['alasan'],1, 1, 'L');
    $no++;
}
$pdf->SetFont('Times','B',7);
$order="SELECT * FROM tbl_return_barang WHERE tgl_return_barang between '$ta' AND '$ak'";
...
$pdf->Cell(24, 0.8,"Jumlah Return Barang",1, 0, '');
