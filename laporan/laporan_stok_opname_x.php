...
$pdf->SetX(1.6);
$pdf->SetFont('Times','i',10);
$pdf->MultiCell(22.5,0.6,"Laporan Stok Opname: ".$ta."- ".$ak,0,'L'); 
...
$pdf->Cell(2, 0.6, 'Kategori', 1, 0, 'C');
$pdf->Cell(2, 0.6, 'Merek', 1, 0, 'C');
$pdf->Cell(2, 0.6, 'Stok Awal', 1, 0, 'C');
$pdf->Cell(2, 0.6, 'P/T', 1, 0, 'C');
$pdf->Cell(3, 0.6, 'Tgl. Stok Opname', 1, 0, 'C');
$pdf->Cell(2, 0.6, 'Stok Gudang', 1, 0, 'C');
$pdf->Cell(2, 0.6, 'Selisih', 1, 0, 'C');
$pdf->Cell(4, 0.6, 'Keterangan', 1, 1, 'C');

$no=1;
$sql="SELECT * FROM tbl_stok_opname WHERE tgl_stok_opname between '$ta' AND '$ak'";
...
while($lihat=mysqli_fetch_array($tampil)){
    ...
    while ($data1=mysqli_fetch_array($sql1)) {
        $pdf->Cell(4, 0.6, $lihat['id_barang']."-".$data1['nama_barang'],1, 0, 'L');
    }
    $pdf->Cell(4, 0.6, $lihat['nama_supplier'],1, 0, 'L');
    $pdf->Cell(2, 0.6, $lihat['nama_kategori'],1, 0, 'L');
    $pdf->Cell(2, 0.6, $lihat['nama_merek'],1, 0, 'L');
    $pdf->Cell(2, 0.6, $lihat['stok'],1, 0, 'L');
    $pdf->Cell(2, 0.6, $lihat['periode']."-".$lihat['tahun'],1, 0, 'L');
    $pdf->Cell(3, 0.6, $lihat['tgl_stok_opname'],1, 0, 'L');
    $pdf->Cell(2, 0.6, $lihat['stok_opname'],1, 0, 'L');
    $pdf->Cell(2, 0.6, $lihat['selisih'],1, 0, 'L');
    $pdf->Cell(4, 0.6, $lihat['keterangan'],1, 1, 'L');
    $no++;
}
$pdf->SetFont('Times','B',7);
$order="SELECT * FROM tbl_stok_opname WHERE tgl_stok_opname between '$ta' AND '$ak'";
...
$pdf->Cell(24, 0.8,"Jumlah Stok Opname",1, 0, '');
